...
class CrossPairwiseImg(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # pair in video
        query_path, query_gt_path, videoStartIndex, videoLength, video_name = self.videoImg_list[index]  # exemplar
        ...
        exemplar_path, exemplar_gt_path, videoStartIndex2, videoLength2, _ = self.videoImg_list[exemplar_index]  # exemplar
        if videoStartIndex != videoStartIndex2 or videoLength != videoLength2:
            raise TypeError('Something wrong')
        other_path, other_gt_path, videoStartIndex3, videoLength3, _ = self.videoImg_list[other_index]  # other
        ...
        if len(self.img_root) > 0:
            ...
            sample = {'exemplar': exemplar, 'exemplar_gt': exemplar_gt, 'query': query, 'query_gt': query_gt,
                  'other': other, 'other_gt': other_gt, 'video_name': video_name}
        return sample
        ...
        for video in video_list:
            ...
            for img in img_list:
                ...
                videoImgGt = (os.path.join(root[0], self.input_folder, video, img + self.img_ext),
                        os.path.join(root[0], self.label_folder, video, img + self.label_ext), self.num_video_frame, len(img_list), video)
